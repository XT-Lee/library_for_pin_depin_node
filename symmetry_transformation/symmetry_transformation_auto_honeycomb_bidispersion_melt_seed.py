...
def workflow(index1,kt1,step,kt_end,linear_compression_ratio,kset,seed_set=9):
    R"""
    Introduction:
        The critical Linear Compression Ratio for honeycomb lattice is 0.816.
    ...
    for i in numpy.linspace(1,num,num):
        ...
        simulation1(linear_compression_ratio,trap_filename,kset,rcut,kt_set,seed_set,file_log,file_gsd,result_filename,png_filename)
        print('this is the '+str(i.astype(int))+'th cycle\n')

    return index.astype(int) #as index_end


def simulation1(linear_compression_ratio,trap_filename,kset,rcut,kt_set,seed_set,file_log,file_gsd,result_filename,png_filename=""):
    R"""
    introduction:
        the simulation shows the melting process 
    ...
    seq=sg.sequence()
    seq.generate_honeycomb(a=a_set,n=[6,12])#initial state! Not traps!
