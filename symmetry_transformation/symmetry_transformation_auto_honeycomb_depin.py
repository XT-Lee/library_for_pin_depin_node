...
def workflow(index1,k1,step,k_end,linear_compression_ratio):
    ...
    for i in numpy.linspace(1,num,num):
        ...
        simulation1(linear_compression_ratio,trap_filename,kset,rcut,file_log,file_gsd,result_filename,png_filename)
        print('this is the '+str(i.astype(int))+'th cycle\n')

    return index.astype(int) #as index_end


def simulation1(linear_compression_ratio,trap_filename,kset,rcut,file_log,file_gsd,result_filename,png_filename):
    R"""
    introduction:
        the simulation shows the depinning process from honeycomb through decreasing trap potential.
    parameters:
