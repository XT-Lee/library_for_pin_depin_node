...
class initial_state_generator:
    ...
    def set_new_gsd_file_2types_by_n_size(self,particle_pointer,n_size,positions,trap_pointer,output_gsd_filename,trap_fill_box=True,perturb=True):
        R"""
        introduction:
    ...
    def set_new_gsd_file_2types_depin_by_box(self,particle_pointer,box,trap_pointer,output_gsd_filename,perturb=True):
        R"""
        introduction:
            Generate a gsd file containing an array of particles and an array of traps.
        ...
        #when the lattice is a parrallelogram, remove the points outside the box 
        pd = pa.static_points_analysis_2d(positions,hide_figure=False)
        pd.cut_edge_of_positions_by_box(positions,bx)#cut_edge_of_positions_by_xylimit(-0.5*bx[0],0.5*bx[0],-0.5*bx[1],0.5*bx[1])
        positions = positions[pd.inbox_positions_bool]#edge_cut_positions_bool
        del pd
        pd = pa.static_points_analysis_2d(trap_positions,hide_figure=False)
        pd.cut_edge_of_positions_by_box(trap_positions,bx)#cut_edge_of_positions_by_xylimit(-0.5*bx[0],0.5*bx[0],-0.5*bx[1],0.5*bx[1])
    ...
    def set_new_gsd_file_2types_by_box_or_n_size(self,particle_pointer,trap_pointer,output_gsd_filename,perturb=True,box=None,n_size=None):#checked right
        R"""
        introduction:
            Generate a gsd file containing an array of particles and an array of traps.
            caution: it only works when particle is type_n, trap is type_n_part, 
