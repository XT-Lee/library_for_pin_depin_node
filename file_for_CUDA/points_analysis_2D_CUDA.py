...
class static_points_analysis_2d:#old_class_name: PointsAnalysis2D
    ...
    def __init__(self,points=None,filename=None,hide_figure = True):
        ...
        if points is None:
            if filename is None:
                ...
                self.points = cp.loadtxt(filename)
                self.points = self.points[:,0:2]
                self.basic_points_analysis()
        else :
            self.points = cp.array(points[:,:2]) 
            self.basic_points_analysis()
    ...
    def basic_points_analysis(self):
        self.voronoi = Voronoi(np.array(self.points.get()))
        self.delaunay = Delaunay(np.array(self.points.get()))
    ...
    def get_ridge_length(self):
        ...
        self.voronoi.ridge_vertices=np.array(self.voronoi.ridge_vertices,dtype=np.int32)#选不了ridge_vertices,不是数组格式
        v1 = np.array(self.voronoi.vertices[self.voronoi.ridge_vertices[:,0],:])
        v2 = np.array(self.voronoi.vertices[self.voronoi.ridge_vertices[:,1],:])
        self.voronoi.ridge_length=distance.cdist(v1,v2,'euclidean')
    ...
    def get_bond_length(self):
        ...
        self.voronoi.ridge_points=np.array(self.voronoi.ridge_points)
        shp=np.shape(self.voronoi.ridge_points)
        self.bond_length = np.zeros((shp[0],shp[1]+1))#(start_point,end_point,length)
        v1 = np.array(self.voronoi.points[self.voronoi.ridge_points[:,0],:])
        v2 = np.array(self.voronoi.points[self.voronoi.ridge_points[:,1],:])
        bond_length_temp=distance.cdist(v1, v2, 'euclidean')
        bond_length_temp=bond_length_temp.diagonal()
        bond_length_temp=np.transpose(bond_length_temp)
        self.bond_length[:,2]=bond_length_temp[:].get()#should I list point pair like small-to-large？
        self.bond_length[:,0:2]=self.voronoi.ridge_points[:]#np.array()
        self.bond_length = cp.array(self.bond_length)
    ...
    def get_first_minima_bond_length_distribution(self,lattice_constant=3.0,hist_cutoff=2,
        ...
        plt.figure()
        count_bins=plt.hist(self.bond_length[:,2].get()/self.lattice_constant,bins=20,range=[0,hist_cutoff])
        
        self._count=count_bins[0]
        self._bins=count_bins[1]
        self.bond_sorted=cp.sort(self.bond_length[:,2]/self.lattice_constant)
        ...
        if method=="local_compare_method":
            while i < i_max-3:#i start from 0, which have to -1;compares i,i+1 and i+2, which have to -2, hence -3
                #cp.where( self.bond_sorted[:]>res.bins[i] & self.bond_sorted[:]< res.bins[i+1] ) 
                #print(self._count[i])
            ...
            self._count = cp.array(self._count)
            self.count_sorted=cp.sort(self._count)
            """
            ...
            if cp.shape(i_bins_for_count_peak_1)[0]>1:
                i_bins_for_count_peak_1 = min(i_bins_for_count_peak_1)
            if cp.shape(i_bins_for_count_peak_2)[0]>1:
            ...
            """
            #list_xy = cp.logical_and(list_x,list_y)
            #self.edge_cut_positions_list = cp.where(==)
        ...
        if not png_filename  is None:
            #plot divide line
            plt.plot([self.bond_first_minima_left,self.bond_first_minima_left],[0,self._count[i].get()],linestyle='--')
    ...
    def get_first_minima_radial_distribution_function(self,rdf,lattice_constant=3.0,png_filename=None):#[x]
        ...
        self._count = cp.array(self._count)
        self._bins = cp.array(self._bins)
        self.count_sorted=cp.sort(self._count)
        ...
        if not png_filename  is None:
            ...
            plt.title("1st peak:"+str( cp.around( cp.median(self.bond_first_neighbour),2) )+"+-"+str( cp.around(cp.std(self.bond_first_neighbour),2) ))
    ...
    def get_coordination_number_conditional(self):
        ...
        #self._particle_id_max=cp.max(self.__coordination_bond[:])#get the largest id of particle to restrain while loop
        #print(self.__coordination_bond)

        cn_max = 12#for honeycomb, cn10 truly appears in Index4340_seed9_frame2!
        self.count_coordination=cp.zeros([cn_max,1])
        
        for id in self.edge_cut_positions_list[0]:
            place=cp.where(self.__coordination_bond[:]==id)
            """
  ...
  File "/home/tplab/xiaotian_file/lxt_code_py/control_table_multi_thread.py", line 13, in <module> (Current frame)
    ...
    def get_nearest_neighbor_dispalcement(self):
        ...
        for id in self.edge_cut_positions_list[0]:
            ...
            place_nb=cp.array(place,dtype=cp.int32) 
            #print(place_nb)
            place_nb[1]=1-place_nb[1]
            #print(place_nb)
            id_nb=self.__coordination_bond[place_nb[0],place_nb[1]].astype(cp.int32)
            sum_nb=cp.sum(id_nb)
            #print(id_nb)#id s 
            
            #print(self.points[id_nb])
            center_nb = cp.average(self.points[id_nb.get()],axis = 0)
            dxy_id = self.points[id.get()]-center_nb
            #print(center_nb)
    ...
    def get_neighbor_cloud_method_voronoi(self,png_filename=None):#checked right
        ...
        new_ticks = cp.linspace(-dis,dis,int(2*dis+1))
    ...
    def get_neighbor_cloud_method_1st_minima_bond(self,png_filename=None):#checked right
        ...
        plt.figure()
        #count=cp.zeros((self.points.shape[0],1))
        for i,j in pairs:
            if self.edge_cut_positions_bool[i]:
                #cp.np.concatenate([array1,array2]) 
    ...
    def draw_bonds_conditional_bond_cloud(self,check=[0.9,2.0],png_filename=None):
        ...
        #add lines for edges
        for i in range(cp.shape(self.bond_length)[0]):
        ...
        #plt.show()
        plt.title("bond_length:"+str(cp.around(check,2))+"um")
        if not png_filename is None:
            plt.savefig(png_filename)
        #plt.close()
    ...
    def __cut_edge_of_positions(self,effective_lattice_constant = 3):
        ...
        #sz = len(self.init_positions)#cp.size
        ...
        list_xmin = cp.array(self.points[:,0] > xmin)
        list_xmax = cp.array(self.points[:,0] < xmax)
        list_ymin = cp.array(self.points[:,1] > ymin)
        list_ymax = cp.array(self.points[:,1] < ymax)

        list_x = cp.logical_and(list_xmin,list_xmax)
        list_y = cp.logical_and(list_ymin,list_ymax)
        list_xy = cp.logical_and(list_x,list_y)

        self.edge_cut_positions_list = cp.where(list_xy)
    ...
    def draw_bonds_conditional_bond(self,check=[0.9,2.0],png_filename=None,nb_change=None,x_unit='(um)',
        ...
        center = cp.array([(xmax+xmin)*0.5,(ymax+ymin)*0.5])
        center_origin_distance = cp.abs(cp.dot(center,center))
        ...
        plt.xlabel('x'+x_unit)
        plt.ylabel('y'+x_unit)
        plt.title("bond_length:"+str(cp.around(check,2))+x_unit)
        """
        the displayed image size will be the smaller one 
        between axis limitation for xlim/ylim or data itself. 
        Hence the effective xlim/ylim should be set smaller than data.
        ...
        for i in range(cp.shape(self.bond_length)[0]):
            if (self.bond_length[i,2] > bond_check[0])&(self.bond_length[i,2] < bond_check[1]) :
                edge = self.bond_length[i,0:2].astype(int)
                pt1,pt2 = [self.points[edge[0]],self.points[edge[1]]]
                line = plt.Polygon([pt1.get(),pt2.get()], closed=None, fill=None, edgecolor='b')
                plt.gca().add_line(line)

        plt.scatter(self.points[:,0].get(),self.points[:,1].get(),color='k')
        """
        particle_ids = cp.linspace(0,self.points.shape[0]-1,self.points.shape[0],dtype=int)
        ...
        if not nb_change is None:
            plt.scatter(self.points[nb_change,0].get(),self.points[nb_change,1].get(),color='orange')
        
        if show_traps:
            traps=cp.loadtxt(trap_filename)
            traps=cp.multiply(traps,LinearCompressionRatio)
            plt.scatter(traps[:,0].get(), 
                   traps[:,1].get(),
                   c='r',marker = 'x')
    ...
    def draw_bonds_conditional_ridge(self,fignum=1,check=[0.2,5.0]):
        #record cutted bond
        count=0
        shp=cp.shape(self.voronoi.ridge_length)
        cutted_temp = cp.zeros((shp[0],2))
        cutted_ridge_temp = cp.zeros((shp[0],1))
        #[x]
        median_temp = cp.median(self.voronoi.ridge_length)
        ...
        #add lines for edges
        for i in range(cp.shape(self.voronoi.ridge_length)[0]):
    ...
    def print_benchmark(self,bond=True,ridge=True):
        if bond :
            ...
            print('median',cp.median(self.bond_length[:,2]))
            print('max',max(self.bond_length[:,2]))
            print('\n')
        if ridge :
            print('ridge')
            print('min',min(self.voronoi.ridge_length))
            print('median',cp.median(self.voronoi.ridge_length))
    ...
    def print_basic_property(self):
        ...
        energy_interaction=epsilon*cp.exp(-kappa)# when rcut=12.5, Uparticle = kBT = 1; rcut=15, Up=0.47
        energy_substrate=0.5*k*cp.multiply(rcut,rcut)
        #pt=cp.exp(1)
    ...
    def get_cluster_vertices(self):
        ...
        c=cp.concatenate([v_index_temp[:,0],v_index_temp[:,1]])
        c=cp.sort(c)
        shp=cp.shape(c)
    ...
    def count_n_polygon_fast(self,n_polygon=6):
        ...
        shp=cp.shape(cv_temp)
        cluster_count=cp.zeros((shp[0]))
        ...
        for i in range(shp[0]):
            ...
            if cv_dis[i,0]==255:
                ...
                list=cp.where(cv_dis[i,:]<check)
                print(list[0])
                #cv_dis[:,list]=255#delete found vertices 
                cv_dis[list,:]=255#delete found vertices 
                count=cp.shape(list[0])
    ...
    def count_honeycomb(self):   
        '''
            ...
            shp=cp.shape(self.cutted_bonds)
            self.linked_triangles = cp.zeros(shp)
            for i in range(shp[0]):
                #1st round search
                index1=cp.where(self.delaunay.simplices[:]==self.cutted_bonds[i,0])
                #2nd round search
                index2=cp.where(self.delaunay.simplices[index1[0]]==self.cutted_bonds[i,1])
                temp=index1[0]
                self.linked_triangles[i]=temp[index2[0]]
            del temp
            #get linked triangles chain
            chain=cp.zeros((shp[0],8))#up to 10-polygon
            chain[:]=-1
            all_chain_count=cp.zeros((shp[0],1))
            ...
            lt_temp=self.linked_triangles[:]#cp.zeros((shp[0],2))
            ...
            while sum(all_chain_count)<tri_count_max:
                if lt_temp[i,0]==-1:
                    ...
                    index1=cp.where((lt_temp[:,0]==self.linked_triangles[i,1])|(lt_temp[:,1]==self.linked_triangles[i,1]))#rightward
                    index_check1=cp.shape(index1[0])
                    if index_check1[0]==0:
        ...
        for i in range(shp[0]):
            if i==0:
                ...
                index1=cp.where((chain[:]==lt_temp[i,0])|(chain[:]==lt_temp[i,1]))
...
class dynamic_points_analysis_2d:#old_class_name: msd
    ...
    def compute_nearest_neighbor_displacements(self,csv_prefix='',bond_cut_off=6):
        ...
        parameters:
            input: 
                trajectory_stable (unit = 1 or um)
        ...
        list_1st_bond_length_framewise = cp.zeros((self.frames,))
        for frame in list_frames:
            ...
            if frame == 0:
                ...
                #ts_id_dxy = cp.concatenate((ts_id_dxy,t_id_dxy), axis=0) #[frame,id,dx,dy]
        ...
        list_ids = cp.unique(ts_id_dxy[:,1])
        id_check = cp.zeros((list_ids.shape[0],),dtype=bool)
        for id in list_ids:
        ...
        """
        average_1st_bond_length = cp.average(list_1st_bond_length_framewise) 
    ...
    def monitor_neighbor_change_event(self,ts_id_dxy,csv_prefix=''):
        ...
        list_id_counts = cp.unique(list_id,return_counts=True) 
        list_id_counts = cp.array(list_id_counts)
        ...
        list_sum_id_nb_stable = list_sum_id_nb[list_sum_id_nb["particle_id"].isin(list_id_nb_not_cut_edge.get())]
        list_sum_id_nb_stable_np = list_sum_id_nb_stable["sum_id_neighbors"].values
        list_sum_id_nb_stable_np = cp.reshape(list_sum_id_nb_stable_np,(int(n_frame_nb_stable),n_particle_nb_stable))
        ...
        sum_id_diff = cp.absolute(cp.array(sum_id_1 - sum_id_0))
        # if sum_id_diff[:,:] != 0 then = 1
        #compare sum_id_neighbors between frame+1 and frame
        #different -> change_neighbor = true
        sum_id_diff = cp.array(sum_id_diff,dtype=bool)
        if_nb_change = cp.array(list_sum_id_nb_stable_np,dtype=bool)
        if_nb_change[0] = False
        if_nb_change[1:] = sum_id_diff
        if_nb_change_int = cp.array(if_nb_change,dtype=int)
        #reshape if_nb_change and plug back into list_sum_id_nb_stable
        """
        if_nb_change_pd = pd.DataFrame(np.reshape(if_nb_change_int.get(),int(n_frame_nb_stable)*n_particle_nb_stable))
        if_nb_change_pd.columns = ['if_nb_change']
        list_sum_id_nb_stable = pd.concat([list_sum_id_nb_stable,if_nb_change_pd],axis=1)
        """
        file_list_sum_id_nb_stable = csv_prefix + 'list_sum_id_nb_stable.csv'
        #list_sum_id_nb_stable.columns = ['frame','particle_id','sum_id_neighbors','if_nb_change']
        #list_sum_id_nb_stable['if_nb_change'] = False
        #to fix error: 'BlockManager' object has no attribute 'columns'
        pdd = cp.array(list_sum_id_nb_stable)
        pdd2 = pd.DataFrame(pdd)
        pdd2.columns = ['frame','particle_id','sum_id_neighbors']
        pdd2.loc[:,'if_nb_change'] = cp.reshape(if_nb_change_int.get(),int(n_frame_nb_stable)*n_particle_nb_stable)
        pd.DataFrame.to_csv(pdd2,file_list_sum_id_nb_stable)
    ...
    def hist_neighbor_change_event(self,if_nb_change_int,n_particle_nb_stable):
        ...
        count_nb_change_event = cp.sum(if_nb_change_int,axis=1)
    ...
    def dynamic_coordination_bond_plot(self,data_name='default_exp',prefix='',
        ...
        """
        #prefix='/home/'+prefix+'/Downloads/'#'/home/tplab/Downloads/'
        #log_prefix='/home/'+prefix+'/hoomd-examples_0/'#'/home/tplab/hoomd-examples_0/'
        #load time steps
        str_index=data_name
...
class mean_square_displacement:
    ...
    example_experiment:
        ...
        time_log = cp.loadtxt(path+'DefaultVideo_5.txt')
    ...
    EXAMPLE:
        ...
        print(cp.shape(msds.result_msd))
    example:
        import points_analysis_2D as pa
    ...
    def compute_msd_normal(self):
        ...
        #k_max = 0
        start = 1
        self.record_msd=cp.zeros((m_max-start+1,2))#[interval m, msd_m]
        
        list_m=cp.arange(start,m_max+1)
        self.record_msd[:,0]=list_m
        for m in list_m:
            self.dm_xyz = self.txyz_stable[m:,:,:] - self.txyz_stable[0,:,:]
            sq_dm_xyz2 = cp.square(self.dm_xyz)
            sum_sq_dm_r2 = cp.sum(sq_dm_xyz2)
    ...
    def compute_atmsd_t_chips(self,interval_max=0.1,msd_per_particle=False):
        ...
        for m in list_m:
            ...
            list_frames = m*cp.arange(0,chips+1)
            ...
            if msd_per_particle:
                id_sum_sq_dm_r2_chips = cp.sum(sq_dm_xyz2,axis=2)
                print(id_sum_sq_dm_r2_chips.shape)
                id_sum_sq_dm_r2 = cp.sum(id_sum_sq_dm_r2_chips,axis=0)
                print(id_sum_sq_dm_r2.shape)
                if 'record_msd_id' in locals():
                    record_msd_id[m-start,:]=id_sum_sq_dm_r2/self.dm_xyz.shape[0]
                else:
                    record_msd_id=cp.zeros((m_max-start+1,self.particles))
                    record_msd_id[m-start,:]=id_sum_sq_dm_r2/self.dm_xyz.shape[0]
    ...
    def compute_nn_msd_X(self,unit='sigma'):
        ...
        for frame in list_frames:
            ...
            t_id_dxy = cp.zeros((id_dxy.shape[0],id_dxy.shape[1]+1))  
            t_id_dxy[:,0] = frame
            t_id_dxy[:,1:] = id_dxy  
            if frame == 0:
                ts_id_dxy = t_id_dxy
    ...
    def plot_msd_t_chips(self,time_log=None,png_filename='msd_chips_long_loglog.png',sigma=False,um_sec=True,lindemann=False):
        ...
        plt.close()
        """
        list_dm_frames = cp.arange(self.dm_xyz.shape[0])
        for i in list_dm_frames:
            #cp.dot()
            self.dm_xyz_tr=cp.transpose(self.dm_xyz)
            cp.matmul(self.dm_xyz),self.dm_xyz_tr)
        """
        
        
        """
        m=0#frame interval
        record_disp2_1 = cp.zeros((self.sp[0]-1,3))
        while m<self.sp[0]-1:#m < N frame
            k=0#frame scanning
            record_disp2_m = cp.zeros((1,3))
            
            while k<self.sp[0]-1-m:
                disp_m = positions[k+m]-positions[k]#199-0 what is the problem?
                disp2_m = cp.dot(cp.transpose(disp_m),disp_m) 
        ...
        self.result_msd = cp.zeros((self.sp[0]-1,1))
...
class displacemnt_field_2D:
    ...
    def get_displacement_field_distribution(self,frame_index,log_mode=False,png_filename=None):
        ...
        rsd = cp.sqrt(square_displacement)

        if log_mode:
            rsd = cp.log10(square_displacement)
            plt.figure()
