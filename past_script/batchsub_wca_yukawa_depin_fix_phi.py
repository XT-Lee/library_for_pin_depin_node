...
type_ns = [3,8,10,11]#72,
ind_st = 7933
ns = 600
index1s = [ind_st,ind_st+ns,ind_st+ns*2,ind_st+ns*3]#7333+70,
"""import symmetry_transformation_v4_3.simulation_controller as sc
wy = sc.simulation_controller_wca_yukawa_traps()
wy.get_lcr0()
print(wy.lcr0)
for i in range(len(type_ns)):
    type_n = type_ns[i]
    index1 = index1s[i]
    wy.generate_simu_index_csv_depin_fix_phi(index1,type_n)"""
for i in range(len(type_ns)):
    type_n = type_ns[i]
    index1 = index1s[i]
    ...
    #wy.generate_initial_state_type_n_particle_type_n_part_trap_scan_csv_fix_phi(output_file_csv)
    ...
    agf.extract_data_from_csv_u_yukawa_r1_U_eq_cnk(output_file_csv,"wca_yukawa_depin_from_type"+str(type_n)+"_"+str(int(index1))+"_p6.csv",6)
